...
impl Bag {
    ...
    pub(super) unsafe fn try_push(&mut self, deferred: Deferred) -> Result<(), Deferred> {
...
impl Global {
    ...
    pub(super) fn collect(&self, guard: &Guard) {
        ...
        let steps = COLLECT_STEPS
            .max(SIZE_HINT.load(Ordering::Relaxed) / 16)
            .min(512);
...
fn entry_offset() -> usize {
    ...
    let local_ref: &Local = unsafe { &*(&local as *const MaybeUninit<Local> as *const Local) };
...
impl IsElement<Local> for Local {
    ...
    unsafe fn element_of(entry: &Entry) -> &Local {
        #[allow(trivial_casts)]
        let local_ptr = (entry as *const Entry as usize - entry_offset()) as *const Local;
        &*local_ptr
    }
